...
use crate::app::state::{AppState, NavSection, Screen, UiMode};

use super::theme::{error_style, muted_style, panel_block};
...
pub fn split_shell(_frame: &mut Frame, area: Rect, state: &AppState) -> (Rect, Rect) {
    let sidebar_width = match state.ui_mode {
        UiMode::Compact => 16,
        UiMode::Overlay => {
...
pub fn render_search_overlay(frame: &mut Frame, state: &AppState) {
    ...
    let block = panel_block(" Search (Ctrl+F close, Esc cancel) ", theme);
...
pub fn render_error_overlay(frame: &mut Frame, state: &AppState) {
    ...
    let block = panel_block(" Error detail (Esc close, j/k scroll) ", theme);
    frame.render_widget(
...
pub fn render_mode_hint(frame: &mut Frame, area: Rect, state: &AppState) {
    ...
    let hint = format!("{mode} • {focus} • Ctrl+F search • m mode");
    frame.render_widget(
...
pub fn nav_label(section: NavSection) -> &'static str {
    match section {
        NavSection::Home => "Home",
        NavSection::Projects => "Projects",
        NavSection::Deployments => "Deployments",
        NavSection::Monitoring => "Monitoring",
        NavSection::Schedules => "Schedules",
    }
}
