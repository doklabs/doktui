...
### 5.2 App Deployment
Deploy from a **GitHub repo** (clone/pull over SSH after connecting an account via OAuth Device Flow in the browser) or a pasted Docker Compose file. Navigation mirrors Dokploy intent: **Servers** = SSH machines, **Apps** = deployments (many apps per server via different remote dirs). GitHub apps can **auto-deploy** by polling commit SHAs while DokTUI is open (not a 24/7 server webhook). DokTUI injects **Traefik routing labels** so your app is reachable via your domain with HTTPS.
